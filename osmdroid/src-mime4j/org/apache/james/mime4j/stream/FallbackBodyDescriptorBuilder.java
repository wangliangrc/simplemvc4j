...
/**
 * Encapsulates the values of the MIME-specific header fields (which starts with
 * <code>Content-</code>).
 */
class FallbackBodyDescriptorBuilder implements BodyDescriptorBuilder {
     ...
     * 
     * @param parent
     *            the descriptor of the parent or <code>null</code> if this is
     *            the root descriptor.
     */
    public FallbackBodyDescriptorBuilder(final String parentMimeType,
    ...
    public BodyDescriptor build() {
        ...
        return new BasicBodyDescriptor(actualMimeType, actualMediaType,
                actualSubType, boundary, actualCharset,
                transferEncoding != null ? transferEncoding : "7bit",
                contentLength);
    }

    /**
     * Should be called for each <code>Content-</code> header field of a MIME
     * message or part.
     * 
     * @param field
     *            the MIME field.
    ...
    private void parseContentType(Field field) throws MimeException {
        ...
        if (main != null
                && ((main.startsWith("multipart/") && b != null) || !main
