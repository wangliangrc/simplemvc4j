...
/**
 * Low level parser for header field elements. The parsing routines of this
 * class are designed to produce near zero intermediate garbage and make no
 * intermediate copies of input data.
 * <p/>
...
public class RawFieldParser {
    ...
    public RawField parseField(final ByteSequence raw) throws MimeException {
        ...
        if (cursor.atEnd()) {
            throw new MimeException(
                    "Invalid MIME field: no name/value separator found: "
                            + raw.toString());
        }
     ...
     * 
     * @param buf
     *            buffer with the sequence of bytes to be parsed
     * @param cursor
     *            defines the bounds and current position of the buffer
     */
    public RawBody parseRawBody(final ByteSequence buf,
    ...
    /**
     * Parses the sequence of bytes containing field parameters delimited with
     * semicolon into a list of {@link NameValuePair}s.
     * 
     * @param buf
     *            buffer with the sequence of bytes to be parsed
     * @param cursor
     *            defines the bounds and current position of the buffer
     */
    public List<NameValuePair> parseParameters(final ByteSequence buf,
            final ParserCursor cursor) {
    ...
    /**
     * Parses the sequence of bytes containing a field parameter delimited with
     * semicolon into {@link NameValuePair}.
     * 
     * @param buf
     *            buffer with the sequence of bytes to be parsed
     * @param cursor
     *            defines the bounds and current position of the buffer
     */
    public NameValuePair parseParameter(final ByteSequence buf,
            final ParserCursor cursor) {
    ...
    /**
     * Extracts from the sequence of bytes a token terminated with any of the
     * given delimiters discarding semantically insignificant whitespace
     ...
     *            defines the bounds and current position of the buffer
     * @param delimiters
     *            set of delimiting characters. Can be <code>null</code> if the
     *            token is not delimited by any character.
     */
    public String parseToken(final ByteSequence buf, final ParserCursor cursor,
    ...
    /**
     * Extracts from the sequence of bytes a value which can be enclosed in
     * quote marks and terminated with any of the given delimiters discarding
     * semantically insignificant whitespace characters and comments.
     * 
     * @param buf
     *            buffer with the sequence of bytes to be parsed
     * @param cursor
     ...
     * @param delimiters
     *            set of delimiting characters. Can be <code>null</code> if the
     *            value is not delimited by any character.
     */
    public String parseValue(final ByteSequence buf, final ParserCursor cursor,
            final BitSet delimiters) {
        StringBuilder dst = new StringBuilder();
        boolean whitespace = false;
        while (!cursor.atEnd()) {
     ...
     * cursor to the closest non-whitespace character.
     * 
     * @param buf
     *            buffer with the sequence of bytes to be parsed
     * @param cursor
    ...
    /**
     * Skips semantically insignificant content if the current position is
     * positioned at the beginning of a comment and moves the cursor past the
     * end of the comment. Nested comments and escaped characters are recognized
    ...
    /**
     * Skips semantically insignificant whitespace characters and comments and
     * moves the cursor to the closest semantically significant non-whitespace
     * character. Nested comments and escaped characters are recognized and
     ...
     *            defines the bounds and current position of the buffer
     */
    public void skipAllWhiteSpace(final ByteSequence buf,
            final ParserCursor cursor) {
        while (!cursor.atEnd()) {
            char current = (char) (buf.byteAt(cursor.getPos()) & 0xff);
            if (CharsetUtil.isWhitespace(current)) {
    ...
    /**
     * Transfers content into the destination buffer until a whitespace
     * character, a comment, or any of the given delimiters is encountered.
     * 
     * @param buf
     *            buffer with the sequence of bytes to be parsed
     * @param cursor
     ...
     * @param delimiters
     *            set of delimiting characters. Can be <code>null</code> if the
     *            value is delimited by a whitespace or a comment only.
     * @param dst
     *            destination buffer
     */
     ...
     *            destination buffer
     */
    public void copyQuotedContent(final ByteSequence buf,
            final ParserCursor cursor, final StringBuilder dst) {
        if (cursor.atEnd()) {
            return;
        }
        int pos = cursor.getPos();
