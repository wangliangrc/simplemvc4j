...
class MimeEntity implements EntityStateMachine {
    ...
    MimeEntity(LineNumberSource lineSource, InputStream instream,
            MimeConfig config, EntityState startState, EntityState endState,
            DecodeMonitor monitor, FieldBuilder fieldBuilder,
        ...
        this.inbuffer = new BufferedLineReaderInputStream(instream, 4 * 1024,
    ...
    MimeEntity(LineNumberSource lineSource, InputStream instream,
            MimeConfig config, EntityState startState, EntityState endState,
            BodyDescriptorBuilder bodyDescBuilder) {
        this(lineSource, instream, config, startState, endState, config
                .isStrictParsing() ? DecodeMonitor.STRICT
                : DecodeMonitor.SILENT, new DefaultFieldBuilder(
                config.getMaxHeaderLen()), bodyDescBuilder);
    }

    MimeEntity(LineNumberSource lineSource, InputStream instream,
            MimeConfig config, BodyDescriptorBuilder bodyDescBuilder) {
        this(lineSource, instream, config, EntityState.T_START_MESSAGE,
    ...
    MimeEntity(LineNumberSource lineSource, InputStream instream,
            FieldBuilder fieldBuilder, BodyDescriptorBuilder bodyDescBuilder) {
        this(lineSource, instream, new MimeConfig(),
                EntityState.T_START_MESSAGE, EntityState.T_END_MESSAGE,
                DecodeMonitor.SILENT, fieldBuilder, bodyDescBuilder);
    }
    ...
    MimeEntity(LineNumberSource lineSource, InputStream instream,
        ...
        this(lineSource, instream, new MimeConfig(),
                EntityState.T_START_MESSAGE, EntityState.T_END_MESSAGE,
                DecodeMonitor.SILENT, new DefaultFieldBuilder(-1),
                bodyDescBuilder);
    }
    ...
    /**
     * Creates an indicative message suitable for display based on the given
     * event and the current state of the system.
     * 
     * @param event
     *            <code>Event</code>, not null
     * @return message suitable for use as a message in an exception or for
     *         logging
     */
    ...
    protected boolean nextField() throws MimeException, IOException {
        ...
        for (;;) {
            ...
            if (maxHeaderCount > 0 && headerCount >= maxHeaderCount) {
                throw new MaxHeaderLimitException(
                        "Maximum header limit exceeded");
            }
            headerCount++;
            fieldBuilder.reset();
            readRawField();
    ...
    public EntityStateMachine advance() throws IOException, MimeException {
        switch (state) {
            ...
            case T_FIELD:
                state = nextField() ? EntityState.T_FIELD
    ...
    private void createMimePartStream() throws MimeException, IOException {
        ...
        try {
            currentMimePartStream = new MimeBoundaryInputStream(inbuffer,
                    boundary, config.isStrictParsing());
        } catch (IllegalArgumentException e) {
            // thrown when boundary is too long
            throw new MimeException(e.getMessage(), e);
        }
        dataStream = new LineReaderInputStreamAdaptor(currentMimePartStream,
                config.getMaxLineLen());
    }
    ...
    private EntityStateMachine nextMessage() {
        ...
        InputStream instream = currentMimePartStream != null ? currentMimePartStream
                : inbuffer;
        instream = decodedStream(instream);
        return nextMimeEntity(EntityState.T_START_MESSAGE,
                EntityState.T_END_MESSAGE, instream);
    }
    ...
    private EntityStateMachine nextMimeEntity(EntityState startState,
        ...
        if (recursionMode == RecursionMode.M_RAW) {
            ...
            MimeEntity mimeentity = new MimeEntity(lineSource, instream,
                    config, startState, endState, monitor, fieldBuilder,
