...
/**
 * <p>
 * Parses MIME (or RFC822) message streams of bytes or characters. The stream is
 * converted into an event stream.
 * <p>
 * <p>
 * Typical usage:
 * </p>
 ...
 * </pre>
 * <p>
 * Instances of {@link MimeTokenStream} are reusable: Invoking the method
 * {@link #parse(InputStream)} resets the token streams internal state. However,
 * they are definitely <em>not</em> thread safe. If you have a multi threaded
 * application, then the suggested use is to have one instance per thread.
 * </p>
 */
public class MimeTokenStream {
    ...
    /**
     * Constructs a standard (lax) stream. Optional validation events will be
     * logged only. Use {@link MimeConfig#setStrictParsing(boolean)} to turn on
     * strict parsing mode and pass the config object to
     * {@link MimeTokenStream#MimeTokenStream(MimeConfig)} to create a stream
     * that strictly validates the input.
     */
    ...
    public MimeTokenStream(final MimeConfig config,
            final DecodeMonitor monitor, final FieldBuilder fieldBuilder,
            final BodyDescriptorBuilder bodyDescBuilder) {
        super();
        ...
        this.monitor = monitor != null ? monitor : (this.config
                .isStrictParsing() ? DecodeMonitor.STRICT
     ...
     * contents. If the {@code MimeTokenStream} has already been in use, resets
     * the streams internal state.
     */
    ...
    /**
     * <p>
     * Instructs the {@code MimeTokenStream} to parse the given content with the
     * content type. The message stream is assumed to have no message header and
     * is expected to begin with a message body. This can be the case when the
     * message content is transmitted using a different transport protocol such
     * as HTTP.
     * </p>
     * <p>
    ...
    private void doParse(InputStream stream, EntityState start) {
        ...
        rootentity = new MimeEntity(lineSource, stream, config, start,
                EntityState.T_END_MESSAGE, monitor, fieldBuilder,
                bodyDescBuilder);
    ...
    /**
     * Gets the current recursion mode. The recursion mode specifies the
     * approach taken to parsing parts. {@link RecursionMode#M_RAW} mode does
     * not parse the part at all. {@link RecursionMode#M_RECURSE} mode
     * recursively parses each mail when an <code>message/rfc822</code> part is
     * encountered; {@link RecursionMode#M_NO_RECURSE} does not.
     * 
    ...
    /**
     * Sets the current recursion. The recursion mode specifies the approach
     * taken to parsing parts. {@link RecursionMode#M_RAW} mode does not parse
     * the part at all. {@link RecursionMode#M_RECURSE} mode recursively parses
     * each mail when an <code>message/rfc822</code> part is encountered;
     * {@link RecursionMode#M_NO_RECURSE} does not.
     * 
     * @param mode
     *            {@link RecursionMode#M_RECURSE}, {@link RecursionMode#M_RAW}
     *            or {@link RecursionMode#M_NO_RECURSE}
     */
    ...
    /**
     * Finishes the parsing and stops reading lines. NOTE: No more lines will be
     * parsed but the parser will still trigger 'end' events to match previously
    ...
    /**
     * This method returns a transfer decoded stream based on the MIME fields
    ...
    /**
     * Gets a reader configured for the current body or body part. The reader
     * will return a transfer and charset decoded stream of characters based on
     * the MIME fields with the standard defaults. This is a conveniance method
     * and relies on {@link #getInputStream()}. Consult the javadoc for that
     * method for known limitations.
     * 
     ...
     * {@link EntityState#T_FIELD}.
     * 
     * @return String with the fields raw contents.
     * @throws IllegalStateException
     *             {@link #getState()} returns another value than
     *             {@link EntityState#T_FIELD}.
     */
     ...
     * 
     * @throws IllegalStateException
     *             The method has been called, although {@link #getState()} was
     *             already {@link EntityState#T_END_OF_STREAM}.
     */
