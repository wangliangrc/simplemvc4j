 ...
 * 
 * Encodes every "required" char plus the dot ".". We encode the dot by default
 * because this is a workaround for some "filter"/"antivirus" "old mua" having
 * issues with dots at the beginning or the end of a qp encode line (maybe a bad
 * dot-destuffing algo).
 */
public class QuotedPrintableOutputStream extends FilterOutputStream {
    ...
    public QuotedPrintableOutputStream(int bufsize, OutputStream out,
            boolean binary) {
        super(out);
    ...
    private void encodeChunk(byte[] buffer, int off, int len)
            throws IOException {
