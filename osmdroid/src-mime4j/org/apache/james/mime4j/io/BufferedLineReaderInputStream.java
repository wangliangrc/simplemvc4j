...
public class BufferedLineReaderInputStream extends LineReaderInputStream {
    ...
    public BufferedLineReaderInputStream(final InputStream instream,
            int buffersize, int maxLineLen) {
        super(instream);
        if (instream == null) {
    ...
    private void expand(int newlen) {
        ...
        int len = bufferLen();
        if (len > 0) {
            System.arraycopy(this.buffer, this.bufpos, newbuffer, this.bufpos,
                    len);
        }
    ...
    public int fillBuffer() throws IOException {
        if (tempBuffer) {
            ...
            if (bufpos != buflen)
                throw new IllegalStateException(
                        "unread only works when a buffer is fully read before the next refill is asked!");
        ...
        if (this.bufpos > 0) { // could swtich to (this.buffer.length / 2) but
                               // needs a 4*boundary capacity, then (instead of
                               // 2).
            int len = bufferLen();
            if (len > 0) {
     ...
     * <p>
     * SUNDAY D.M., 1990, A very fast substring search algorithm, Communications
     * of the ACM . 33(8):132-142.
     * </p>
     */
    public int indexOf(final byte[] pattern, int off, int len) {
        ...
        if (off < this.bufpos || len < 0 || off + len > this.buflen) {
            throw new IndexOutOfBoundsException("looking for " + off + "("
                    + len + ")" + " in " + bufpos + "/" + buflen);
        }
    ...
    public int byteAt(int pos) {
        if (pos < this.bufpos || pos > this.buflen) {
            throw new IndexOutOfBoundsException("looking for " + pos + " in "
                    + bufpos + "/" + buflen);
        }
