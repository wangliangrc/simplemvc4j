...
/**
 * Stream that constrains itself to a single MIME body part. After the stream
 * ends (i.e. read() returns -1) {@link #isLastPart()} can be used to determine
 * if a final boundary has been seen or not.
 */
public class MimeBoundaryInputStream extends LineReaderInputStream {
    ...
    /**
     * Store the first buffer length. Used to distinguish between an empty
     * preamble and no preamble.
     */
    ...
    public MimeBoundaryInputStream(
            ...
            final String boundary, final boolean strict) throws IOException {
        super(inbuffer);
     ...
     */
    public MimeBoundaryInputStream(
            final BufferedLineReaderInputStream inbuffer, final String boundary)
            throws IOException {
    ...
    private void verifyEndOfStream() throws IOException {
        if (strict && eof && !atBoundary) {
            throw new MimeIOException(new MimeException(
    ...
    private int fillBuffer() throws IOException {
        ...
        for (;;) {
            ...
            // Make sure the boundary is either at the very beginning of the
            // buffer
            // or preceded with LF
            ...
            for (;;) {
                if (buffer.length() > 1) {
                    ...
                    if (ch1 == '\r' && ch2 == '\n') {
                        ...
                        // ignoring everything in a line starting with a
                        // boundary.
                        buffer.skip(1);
                    }
    ...
    public String toString() {
        final StringBuilder buffer = new StringBuilder(
                "MimeBoundaryInputStream, boundary ");
        for (byte b : boundary) {
