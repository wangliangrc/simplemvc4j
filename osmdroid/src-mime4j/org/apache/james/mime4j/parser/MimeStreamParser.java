...
public class MimeStreamParser {
    ...
    public MimeStreamParser(final MimeConfig config,
        ...
        this(new MimeTokenStream(config != null ? config.clone()
                : new MimeConfig(), monitor, bodyDescBuilder));
    }
    ...
    /**
     * Determines whether this parser automatically decodes body content based
     * on the on the MIME fields with the standard defaults.
     */
    ...
    /**
     * Defines whether parser should automatically decode body content based on
     * the on the MIME fields with the standard defaults.
     */
    ...
    /**
     * Parses a stream of bytes containing a MIME message. Please note that if
     * the {@link MimeConfig} associated with the mime stream returns a not null
     * Content-Type value from its {@link MimeConfig#getHeadlessParsing()}
     * method, the message is assumed to have no head section and the headless
     * parsing mode will be used.
     * 
     * @param instream
     *            the stream to parse.
    ...
    public void parse(InputStream instream) throws MimeException, IOException {
        ...
        if (config.getHeadlessParsing() != null) {
            Field contentType = mimeTokenStream.parseHeadless(instream,
                    config.getHeadlessParsing());
    ...
    /**
     * Enables raw mode. In raw mode all future entities (messages or body
     * parts) in the stream will be reported to the
     * {@link ContentHandler#raw(InputStream)} handler method only. The stream
     * will contain the entire unparsed entity contents including header fields
     * and whatever is in the body.
     */
    public void setRaw() {
    ...
    /**
     * Enables flat mode. In flat mode rfc822 parts are not recursively parsed
     * and multipart content is handled as a single "simple" stream.
     */
    public void setFlat() {
     ...
     * parsed but the parser will still call
     * {@link ContentHandler#endMultipart()},
     * {@link ContentHandler#endBodyPart()}, {@link ContentHandler#endMessage()}
     * , etc to match previous calls to
    ...
    /**
     * Sets the <code>ContentHandler</code> to use when reporting parsing
     * events.
     * 
     * @param h
