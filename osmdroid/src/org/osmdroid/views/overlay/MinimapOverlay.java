...
/**
 * Draws a mini-map as an overlay layer. It currently uses its own
 * MapTileProviderBasic or a tile provider supplied to it. Do NOT share a tile
 * provider amongst multiple tile drawing overlays - it will create an
 * under-sized cache.
 * 
 * @author Marc Kurtz
 * 
...
public class MinimapOverlay extends TilesOverlay {
    ...
    // The Mercator coordinates of the map tile area we are interested in the
    // target zoom level
    ...
    /**
     * Creates a {@link MinimapOverlay} with the supplied tile provider. The
     * {@link Handler} passed in is typically the same handler being used by the
     * main map. The {@link MapTileProviderBase} passed in cannot be the same
     * tile provider used in the {@link TilesOverlay}, it must be a new
     * instance.
     * 
     * @param pContext
     *            a context
    ...
    public MinimapOverlay(final Context pContext,
            final Handler pTileRequestCompleteHandler,
        ...
        mTileProvider
                .setTileRequestCompleteHandler(pTileRequestCompleteHandler);
    ...
    /**
     * Creates a {@link MinimapOverlay} that uses its own
     * {@link MapTileProviderBasic}. The {@link Handler} passed in is typically
     * the same handler being used by the main map.
     * 
     * @param pContext
     *            a context
     * @param tileRequestCompleteHandler
    ...
    public MinimapOverlay(final Context pContext,
            final Handler pTileRequestCompleteHandler) {
        this(pContext, pTileRequestCompleteHandler, new MapTileProviderBasic(
                pContext));
    }
    ...
    @Override
    protected void draw(final Canvas pC, final MapView pOsmv,
        ...
        // Make sure the zoom level difference isn't below the minimum zoom
        // level
        if (zoomLevel - getZoomDifference() < mTileProvider
                .getMinimumZoomLevel()) {
        ...
        // Limit the area we are interested in for tiles to be the MAP_WIDTH by
        // MAP_HEIGHT and
        // centered on the center of the screen
        mTileArea.set(mTileArea.centerX() - (getWidth() / 2),
                mTileArea.centerY() - (getHeight() / 2), mTileArea.centerX()
                        + (getWidth() / 2), mTileArea.centerY()
                        + (getHeight() / 2));
        ...
        mMiniMapCanvasRect.set(mViewportRect.right - getPadding() - getWidth(),
                ...
                mViewportRect.right - getPadding(), mViewportRect.bottom
                        - getPadding());
        ...
        pC.drawRect(mMiniMapCanvasRect.left - 2, mMiniMapCanvasRect.top - 2,
                mMiniMapCanvasRect.right + 2, mMiniMapCanvasRect.bottom + 2,
                mPaint);

        super.drawTiles(pC, projection.getZoomLevel()
                - miniMapZoomLevelDifference, projection.getTileSizePixels(),
                mTileArea);
    }

    @Override
    protected void onTileReadyToDraw(final Canvas c,
            final Drawable currentMapTile, final Rect tileRect) {

        // Get the offsets for where to draw the tiles relative to where the
        // minimap is located
        final int xOffset = (tileRect.left - mTileArea.left)
                + (mMiniMapCanvasRect.left);
        final int yOffset = (tileRect.top - mTileArea.top)
                + (mMiniMapCanvasRect.top);
    ...
    public boolean onSingleTapUp(final MotionEvent pEvent,
        ...
        if (mMiniMapCanvasRect.contains((int) pEvent.getX()
                + mViewportRect.left - mWorldSize_2, (int) pEvent.getY()
                + mViewportRect.top - mWorldSize_2)) {
            return true;
        }
    ...
    /**
     * Sets the number of pixels from the lower-right corner to offset the
     * mini-map
     * 
     * @param padding
    ...
    /**
     * Gets the number of pixels from the lower-right corner to offset the
     * mini-map
     * 
