...
final class CustomConcurrentHashMap {
    ...
    static class Impl<K, V, E> extends AbstractMap<K, V> implements
        ...
        final class Segment extends ReentrantLock {
             ...
             * selected uses of volatiles to ensure that completed write
             * operations performed by other threads are noticed. For most
             * purposes, the "count" field, tracking the number of elements,
             * serves as that volatile variable ensuring visibility. This is
             * convenient because this field needs to be read in many read
             * operations anyway:
             * 
             * - All (unsynchronized) read operations must first read the
             * "count" field, and should not look at table entries if it is 0.
             * 
             * - All (synchronized) write operations should write to the "count"
             * field after structurally changing any bin. The operations must
             * not take any action that could even momentarily cause a
             * concurrent read operation to see inconsistent data. This is made
             * easier by the nature of the read operations in Map. For example,
             * no operation can reveal that the table has grown but the
             * threshold has not yet been updated, so there are no atomicity
             * requirements for this with respect to reads.
             * 
             * As a guide, all critical volatile reads and writes to the count
    ...
    static class ComputingImpl<K, V, E> extends Impl<K, V, E> {
        ...
        public V get(Object k) {
             ...
             * of a user calling get() instead of the new API and the risks
             * inherent in adding a new API outweigh this little hole.
             */
            @SuppressWarnings("unchecked")
            K key = (K) k;
    ...
    static class SimpleInternalEntry<K, V> {
        ...
        SimpleInternalEntry(K key, int hash, @Nullable V value,
                SimpleInternalEntry<K, V> next) {
            this.key = key;
            this.hash = hash;
            this.value = value;
