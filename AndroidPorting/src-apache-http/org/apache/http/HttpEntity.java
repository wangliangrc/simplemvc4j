...
public interface HttpEntity {
     ...
     * method is to indicate whether chunked encoding should be used when the
     * entity is sent. For entities that are received, it can also indicate
     * whether the entity was received with chunked encoding. <br/>
    ...
    /**
     * Obtains the Content-Type header, if known. This is the header that should
     * be used when sending the entity, or the one that was received with the
     * entity. It can include a charset attribute.
     * 
     * @return the Content-Type header for this entity, or <code>null</code> if
     *         the content type is unknown
     */
    Header getContentType();

    /**
     * Obtains the Content-Encoding header, if known. This is the header that
     * should be used when sending the entity, or the one that was received with
     * the entity. Wrapping entities that modify the content encoding should
     * adjust this header accordingly.
     * 
     * @return the Content-Encoding header for this entity, or <code>null</code>
     ...
     * to return the same InputStream object more than once. Entities that are
     * not {@link #isRepeatable repeatable} will throw an exception if this
     ...
     *             if this entity is not repeatable and the stream has already
     ...
     * entities should return <code>true</code> until the content has been
     * consumed, <code>false</code> afterwards. Self-contained entities should
     * return <code>false</code>. Wrapping entities should delegate this call to
     * the wrapped entity. <br/>
     ...
     * {@link #consumeContent consumeContent} has been called. If a streamed
     * entity can not detect whether the stream has been read to EOF, it should
     * return <code>true</code> until {@link #consumeContent consumeContent} is
     * called.
     * 
     ...
     * #finish() in the next major release. <br/>
     ...
     * allocated resources as a result of this method invocation. Content
     * streaming entities are also expected to dispose of the remaining content,
     * if any. Wrapping entities should delegate this call to the wrapped
     * entity. <br/>
     * This method is of particular importance for entities being received from
     * a {@link HttpConnection connection}. The entity needs to be consumed
     * completely in order to re-use the connection with keep-alive.
     * 
     * @throws IOException
     *             if an I/O error occurs. This indicates that connection
