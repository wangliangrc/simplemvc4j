...
public final class BasicHttpProcessor implements HttpProcessor,
        HttpRequestInterceptorList, HttpResponseInterceptorList, Cloneable {
    ...
    /**
     * Same as {@link #addRequestInterceptor(HttpRequestInterceptor)
    ...
    public int getRequestInterceptorCount() {
        return (this.requestInterceptors == null) ? 0
                : this.requestInterceptors.size();
    }

    // non-Javadoc, see interface HttpRequestInterceptorList
    public HttpRequestInterceptor getRequestInterceptor(int index) {

        if ((this.requestInterceptors == null) || (index < 0)
    ...
    /**
     * Same as {@link #addResponseInterceptor(HttpResponseInterceptor)
    ...
    public int getResponseInterceptorCount() {
        return (this.responseInterceptors == null) ? 0
                : this.responseInterceptors.size();
    }

    // non-Javadoc, see interface HttpResponseInterceptorList
    public HttpResponseInterceptor getResponseInterceptor(int index) {

        if ((this.responseInterceptors == null) || (index < 0)
    ...
    /**
     * Sets the interceptor lists. First, both interceptor lists maintained by
     * this processor will be cleared. Subsequently, elements of the argument
     * list that are request interceptors will be added to the request
     * interceptor list. Elements that are response interceptors will be added
     * to the response interceptor list. Elements that are both request and
     * response interceptor will be added to both lists. Elements that are
     * neither request nor response interceptor will be ignored.
     * 
     * @param list
     *            the list of request and response interceptors from which to
     *            initialize
     */
    ...
    public void process(final HttpRequest request, final HttpContext context)
        ...
        if (this.requestInterceptors != null) {
            for (int i = 0; i < this.requestInterceptors.size(); i++) {
                HttpRequestInterceptor interceptor = (HttpRequestInterceptor) this.requestInterceptors
                        .get(i);
    ...
    public void process(final HttpResponse response, final HttpContext context)
        ...
        if (this.responseInterceptors != null) {
            for (int i = 0; i < this.responseInterceptors.size(); i++) {
                HttpResponseInterceptor interceptor = (HttpResponseInterceptor) this.responseInterceptors
                        .get(i);
    ...
    protected void copyInterceptors(final BasicHttpProcessor target) {
        ...
        if (this.responseInterceptors != null) {
            target.responseInterceptors = new ArrayList(
