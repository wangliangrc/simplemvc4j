...
/**
 * Provides access to version information for HTTP components. Instances of this
 * class provide version information for a single module or informal unit, as
 * explained <a
 * href="http://wiki.apache.org/jakarta-httpclient/HttpComponents">here</a>.
 * Static methods are used to extract version information from property files
 * that are automatically packaged with HTTP component release JARs. <br/>
 * All available version information is provided in strings, where the string
 * format is informal and subject to change without notice. Version information
 * is provided for debugging output and interpretation by humans, not for
...
public class VersionInfo {
     ...
     */
    protected VersionInfo(String pckg, String module, String release,
            String time, String clsldr) {
        if (pckg == null) {
    ...
    /**
     * Obtains the package name. The package name identifies the module or
     * informal unit.
     * 
    ...
    /**
     * Obtains the name of the versioned module or informal unit. This data is
     * read from the version information for the package.
     * 
    ...
    /**
     * Obtains the release of the versioned module or informal unit. This data
     * is read from the version information for the package.
     * 
    ...
    /**
     * Obtains the timestamp of the versioned module or informal unit. This data
     * is read from the version information for the package.
     * 
    ...
    /**
     * Obtains the classloader used to read the version information. This is
     * just the <code>toString</code> output of the classloader, since the
     * version information should not keep a reference to the classloader
    ...
    public String toString() {
        StringBuffer sb = new StringBuffer(20 + infoPackage.length()
                + infoModule.length() + infoRelease.length()
                + infoTimestamp.length() + infoClassloader.length());

        sb.append("VersionInfo(").append(infoPackage).append(':')
                .append(infoModule);
     ...
     * @param clsldr
     *            the classloader to load from, or <code>null</code> for the
     *            thread context classloader
     * 
     * @return the version information for all packages found, never
     *         <code>null</code>
     */
     ...
     * 
     * @param pckg
     *            the package for which to load version information, for example
     *            "org.apache.http". The package name should NOT end with a dot.
     * @param clsldr
     *            the classloader to load from, or <code>null</code> for the
     *            thread context classloader
     * 
    ...
    public final static VersionInfo loadVersionInfo(final String pckg,
        ...
        try {
            ...
            InputStream is = clsldr.getResourceAsStream(pckg.replace('.', '/')
     ...
     * @param info
     *            the map from string keys to string values, for example
    ...
    protected final static VersionInfo fromMap(String pckg, Map info,
        ...
        if (info != null) {
            ...
            if ((release != null)
                    && ((release.length() < 1) || (release
                            .equals("${pom.version}"))))
                release = null;

            timestamp = (String) info.get(PROPERTY_TIMESTAMP);
            if ((timestamp != null)
                    && ((timestamp.length() < 1) || (timestamp
                            .equals("${mvn.timestamp}"))))
                timestamp = null;
        } // if info
