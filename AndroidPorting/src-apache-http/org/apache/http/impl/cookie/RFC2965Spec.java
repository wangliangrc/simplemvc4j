...
public class RFC2965Spec extends RFC2109Spec {
    ...
    public RFC2965Spec(final String[] datepatterns, boolean oneHeader) {
        ...
        registerAttribHandler(ClientCookie.PORT_ATTR,
                new RFC2965PortAttributeHandler());
        registerAttribHandler(ClientCookie.COMMENTURL_ATTR,
                new RFC2965CommentUrlAttributeHandler());
        registerAttribHandler(ClientCookie.DISCARD_ATTR,
                new RFC2965DiscardAttributeHandler());
        registerAttribHandler(ClientCookie.VERSION_ATTR,
                new RFC2965VersionAttributeHandler());
    }

    private BasicClientCookie createCookie(final String name,
            final String value, final CookieOrigin origin) {
    ...
    private BasicClientCookie createCookie2(final String name,
            final String value, final CookieOrigin origin) {
    ...
    public List<Cookie> parse(final Header header, CookieOrigin origin)
        ...
        for (HeaderElement headerelement : elems) {
            ...
            // Eliminate duplicate attributes. The first occurrence takes
            // precedence
            // See RFC2965: 3.2 Origin Server Role
            Map<String, NameValuePair> attribmap = new HashMap<String, NameValuePair>(
                    attribs.length);
            for (int j = attribs.length - 1; j >= 0; j--) {
                NameValuePair param = attribs[j];
                attribmap.put(param.getName().toLowerCase(Locale.ENGLISH),
                        param);
            }
    ...
    protected void formatCookieAsVer(final CharArrayBuffer buffer,
        ...
        if (cookie instanceof ClientCookie) {
            // Test if the port attribute as set by the origin server is not
            // blank
            String s = ((ClientCookie) cookie)
                    .getAttribute(ClientCookie.PORT_ATTR);
            if (s != null) {
                buffer.append("; $Port");
                buffer.append("=\"");
                if (s.trim().length() > 0) {
    ...
    private static CookieOrigin adjustEffectiveHost(final CookieOrigin origin) {
        ...
        if (isLocalHost) {
            host += ".local";
            return new CookieOrigin(host, origin.getPort(), origin.getPath(),
                    origin.isSecure());
        } else {
            return origin;
        }
