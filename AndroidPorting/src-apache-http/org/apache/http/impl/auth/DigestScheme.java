 ...
 * {@link org.apache.http.auth.params.AuthPNames#CREDENTIAL_CHARSET credential
 * charset} parameter. Since the digest username is included as clear text in
 * the generated Authentication header, the charset of the username must be
 * compatible with the
 * {@link org.apache.http.params.CoreProtocolPNames#HTTP_ELEMENT_CHARSET http
 * element charset}.
 * </p>
 * 
...
public class DigestScheme extends RFC2617Scheme {
    ...
    private String createDigest(final Credentials credentials)
        ...
        StringBuilder tmp = new StringBuilder(uname.length() + realm.length()
                + pwd.length() + 2);
        tmp.append(uname);
        tmp.append(':');
        tmp.append(realm);
        ...
        if (algorithm.equalsIgnoreCase("MD5-sess")) { // android-changed: ignore
                                                      // case
            ...
            String tmp2 = encode(md5Helper.digest(EncodingUtils.getBytes(a1,
                    charset)));
            ...
            tmp3.append(cnonce);
            a1 = tmp3.toString();
        } else if (!algorithm.equalsIgnoreCase("MD5")) { // android-changed:
                                                         // ignore case
            throw new AuthenticationException("Unhandled algorithm "
                    + algorithm + " requested");
        }
        String md5a1 = encode(md5Helper.digest(EncodingUtils.getBytes(a1,
                charset)));
        ...
        if (qopVariant == QOP_MISSING) {
            ...
            StringBuilder tmp2 = new StringBuilder(md5a1.length()
                    + nonce.length() + NC.length() + cnonce.length()
        ...
        String serverDigest = encode(md5Helper.digest(EncodingUtils
