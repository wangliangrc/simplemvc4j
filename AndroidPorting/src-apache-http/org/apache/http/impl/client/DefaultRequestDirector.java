...
/**
 * Default implementation of {@link RequestDirector}. <br/>
...
public class DefaultRequestDirector implements RequestDirector {
    ...
    public DefaultRequestDirector(final HttpRequestExecutor requestExec,
        ...
        this.maxRedirects = this.params.getIntParameter(
                ClientPNames.MAX_REDIRECTS, 100);
    ...
    protected void rewriteRequestURI(final RequestWrapper request,
    ...
    public HttpResponse execute(HttpHost target, HttpRequest request,
        ...
        try {
            while (!done) {
                ...
                Object userToken = context
                        .getAttribute(ClientContext.USER_TOKEN);

                // Allocate connection if needed
                if (managedConn == null) {
                    ClientConnectionRequest connRequest = connManager
                            .requestConnection(route, userToken);
                    if (orig instanceof AbortableHttpRequest) {
                        ((AbortableHttpRequest) orig)
                                .setConnectionRequest(connRequest);
                    }
                ...
                // BEGIN android-added
                    ...
                    // b/3241899 set the per request timeout parameter on reused
                    // connections
                    managedConn.setSocketTimeout(HttpConnectionParams
                            .getSoTimeout(params));
                }
                // END android-added
                ...
                while (retrying) {
                    ...
                    try {
                        if (this.log.isDebugEnabled()) {
                            this.log.debug("Attempt " + execCount
                                    + " to execute request");
                        }
                ...
                if (reuse) {
                    ...
                    long duration = keepAliveStrategy.getKeepAliveDuration(
                            response, context);
                    managedConn
                            .setIdleDuration(duration, TimeUnit.MILLISECONDS);
                }
                ...
                if (followup == null) {
                    ...
                    if (reuse) {
                        ...
                        // Make sure the response body is fully consumed, if
                        // present
    ...
    /**
     * Returns the connection back to the connection manager and prepares for
     * retrieving a new connection during the next request.
     */
    ...
    /**
     * Determines the route for a request. Called by {@link #execute} to
     * determine the route for either the original or a followup request.
     * 
     * @param target
     ...
     * @param context
     *            the context to use for the execution, never <code>null</code>
     * 
     ...
     */
    protected HttpRoute determineRoute(HttpHost target, HttpRequest request,
        ...
        if (target == null) {
            // BEGIN android-changed
            // If the URI was malformed, make it obvious where there's no host
            // component
            String scheme = null;
            String host = null;
            String path = null;
        ...
        do {
            ...
            switch (step) {
                ...
                case HttpRouteDirector.TUNNEL_PROXY: {
                    ...
                    final int hop = fact.getHopCount() - 1; // the hop to
                                                            // establish
                    boolean secure = createTunnelToProxy(route, hop, context);
                    this.log.debug("Tunnel to proxy created.");
                    managedConn.tunnelProxy(route.getHopTarget(hop), secure,
                            this.params);
                }
                    break;
    ...
    /**
     * Creates a tunnel to the target server. The connection must be established
     * to the (last) proxy. A CONNECT request for tunnelling through the proxy
     * will be created and sent, the response received and checked. This method
     * does <i>not</i> update the connection with information about the tunnel,
     ...
     *         <code>false</code> otherwise. The implementation here always
     *         returns <code>false</code>, but derived classes may override.
     * 
     * @throws HttpException
     *             in case of a problem
     * @throws IOException
        ...
        while (!done) {
            ...
            if (creds != null) {
                if (authScope != null || !authScheme.isConnectionBased()) {
                    try {
                        connect.addHeader(authScheme.authenticate(creds,
                                connect));
                    } catch (AuthenticationException ex) {
                        if (this.log.isErrorEnabled()) {
                            this.log.error("Proxy authentication error: "
                                    + ex.getMessage());
                        }
                    }
                }
            ...
            if (credsProvider != null
                    && HttpClientParams.isAuthenticating(params)) {
                if (this.proxyAuthHandler.isAuthenticationRequested(response,
                        context)) {

                    this.log.debug("Proxy requested authentication");
                    Map<String, Header> challenges = this.proxyAuthHandler
                            .getChallenges(response, context);
                    try {
                        processChallenges(challenges, this.proxyAuthState,
                                this.proxyAuthHandler, response, context);
                    } catch (AuthenticationException ex) {
                        if (this.log.isWarnEnabled()) {
                            this.log.warn("Authentication error: "
                                    + ex.getMessage());
                            break;
                        }
                    }
    ...
    /**
     * Creates a tunnel to an intermediate proxy. This method is <i>not</i>
     * implemented in this class. It just throws an exception here.
     * 
     * @param route
     *            the route to establish
     * @param hop
     *            the hop in the route to establish now.
     *            <code>route.getHopTarget(hop)</code> will return the proxy to
     *            tunnel to.
     * @param context
     *            the context for request execution
     * 
     * @return <code>true</code> if the partially tunnelled connection is
    ...
    /**
     * Creates the CONNECT request for tunnelling. Called by
     * {@link #createTunnelToTarget createTunnelToTarget}.
     * 
     * @param route
     *            the route to establish
     * @param context
    ...
    protected HttpRequest createConnectRequest(HttpRoute route,
        ...
        if (port < 0) {
            Scheme scheme = connManager.getSchemeRegistry().getScheme(
    ...
    protected RoutedRequest handleResponse(RoutedRequest roureq,
        ...
        if (HttpClientParams.isRedirecting(params)
            ...
            HttpHost newTarget = new HttpHost(uri.getHost(), uri.getPort(),
                    uri.getScheme());
        ...
        if (credsProvider != null && HttpClientParams.isAuthenticating(params)) {
            ...
            if (this.targetAuthHandler.isAuthenticationRequested(response,
                    context)) {
                ...
                Map<String, Header> challenges = this.targetAuthHandler
                        .getChallenges(response, context);
                try {
                    processChallenges(challenges, this.targetAuthState,
                            this.targetAuthHandler, response, context);
                } catch (AuthenticationException ex) {
                    if (this.log.isWarnEnabled()) {
                        this.log.warn("Authentication error: "
                                + ex.getMessage());
    ...
    /**
     * Shuts down the connection. This method is called from a
     * <code>catch</code> block in {@link #execute execute} during exception
     * handling.
     */
    ...
    private void processChallenges(final Map<String, Header> challenges,
            final AuthState authState, final AuthenticationHandler authHandler,
        ...
        if (authScheme == null) {
            ...
            authScheme = authHandler
                    .selectScheme(challenges, response, context);
