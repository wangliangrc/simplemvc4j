...
public abstract class AbstractAuthenticationHandler implements
    ...
    private static final List<String> DEFAULT_SCHEME_PRIORITY = Arrays
            .asList(new String[] { "ntlm", "digest", "basic" });

    public AbstractAuthenticationHandler() {
        super();
    }

    protected Map<String, Header> parseChallenges(final Header[] headers)
    ...
    public AuthScheme selectScheme(final Map<String, Header> challenges,
            final HttpResponse response, final HttpContext context)
            throws AuthenticationException {

        AuthSchemeRegistry registry = (AuthSchemeRegistry) context
        ...
        for (int i = 0; i < authPrefs.size(); i++) {
            ...
            if (challenge != null) {
                ...
                try {
                    authScheme = registry.getAuthScheme(id,
                            response.getParams());
                    break;
                } catch (IllegalStateException e) {
                    if (this.log.isWarnEnabled()) {
                        this.log.warn("Authentication scheme " + id
                                + " not supported");
                        // Try again
                    }
                }
                ...
                if (this.log.isDebugEnabled()) {
                    this.log.debug("Challenge for " + id
