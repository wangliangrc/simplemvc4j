...
/**
 * A pool entry for use by connection manager implementations. Pool entries work
 * in conjunction with an {@link AbstractClientConnAdapter adapter}. The adapter
 * is handed out to applications that obtain a connection. The pool entry stores
 * the underlying connection and tracks the {@link HttpRoute route} established.
 * The adapter delegates methods for establishing the route to it's pool entry. <br/>
 * If the managed connections is released or revoked, the adapter gets
 * disconnected, but the pool entry still contains the underlying connection and
...
public abstract class AbstractPoolEntry {
     ...
     * 
     * @param connOperator
     *            the Connection Operator for this entry
     * @param route
     *            the planned route for the connection, or <code>null</code>
     */
     ...
     * @param context
     *            the context for opening the connection
     * @param params
    ...
    public void open(HttpRoute route, HttpContext context, HttpParams params)
        ...
        connOperator.openConnection(this.connection, (proxy != null) ? proxy
                : route.getTargetHost(), route.getLocalAddress(), context,
                params);
    ...
    /**
     * Tracks tunnelling of the connection to the target. The tunnel has to be
     * established outside by sending a CONNECT request to the (last) proxy.
     * 
     * @param secure
    ...
    public void tunnelTarget(boolean secure, HttpParams params)
        ...
        this.connection.update(null, tracker.getTargetHost(), secure, params);
     ...
     * be established outside by sending a CONNECT request to the previous
     * proxy.
     * 
     * @param next
     *            the proxy to which the tunnel was established. See
     *            {@link org.apache.http.conn.ManagedClientConnection#tunnelProxy
     *            ManagedClientConnection.tunnelProxy} for details.
     * @param secure
     *            <code>true</code> if the tunnel should be considered secure,
     *            <code>false</code> otherwise
     * @param params
    ...
    public void layerProtocol(HttpContext context, HttpParams params)
        ...
        connOperator.updateSecureConnection(this.connection, target, context,
                params);
     ...
     * 
     * If {@link #open(HttpRoute, HttpContext, HttpParams)} is in progress, this
     * will cause that open to possibly throw an {@link IOException}.
     */
