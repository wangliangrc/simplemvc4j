 ...
 * @version $Revision: 673450 $ $Date: 2008-07-02 10:35:05 -0700 (Wed, 02 Jul
 *          2008) $
 * 
 * @since 4.0
 */
public class DefaultClientConnection extends SocketHttpClientConnection
    ...
    /**
     * Force-closes this connection. If the connection is still in the process
     * of being open (the method {@link #opening opening} was already called but
     * {@link #openCompleted openCompleted} was not), the associated socket that
     * is being connected to a remote address will be closed. That will
     * interrupt a thread that is blocked on connecting the socket. If the
     * connection is not yet open, this will prevent the connection from being
     * opened.
     * 
     * @throws IOException
    ...
    @Override
    protected SessionInputBuffer createSessionInputBuffer(final Socket socket,
            int buffersize, final HttpParams params) throws IOException {
        SessionInputBuffer inbuffer = super.createSessionInputBuffer(socket,
                buffersize, params);
        if (wireLog.isDebugEnabled()) {
            inbuffer = new LoggingSessionInputBuffer(inbuffer,
                    new Wire(wireLog));
        }
        return inbuffer;
    }
    ...
    protected SessionOutputBuffer createSessionOutputBuffer(
            ...
            throws IOException {
        SessionOutputBuffer outbuffer = super.createSessionOutputBuffer(socket,
                buffersize, params);
        if (wireLog.isDebugEnabled()) {
    ...
    // non-javadoc, see interface OperatedClientConnection
    public void update(Socket sock, HttpHost target, boolean secure,
