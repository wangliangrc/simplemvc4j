...
public class RouteSpecificPool {
    ...
    /**
     * The list of free entries. This list is managed LIFO, to increase idle
     * times and allow for closing connections that are not really needed.
     */
    ...
    /**
     * Indicates whether this pool is unused. A pool is unused if there is
     * neither an entry nor a waiting thread. All entries count, not only the
     * free but also the allocated ones.
     * 
     * @return <code>true</code> if this pool is unused, <code>false</code>
     *         otherwise
     */
    ...
    /**
     * Obtains the number of entries. This includes not only the free entries,
     * but also those that have been created and are currently issued to an
     * application.
     * 
    ...
    public BasicPoolEntry allocEntry(final Object state) {
        if (!freeEntries.isEmpty()) {
            ListIterator<BasicPoolEntry> it = freeEntries
                    .listIterator(freeEntries.size());
     ...
     * 
     * @param entry
     *            the entry obtained from {@link #allocEntry allocEntry} or
     *            presented to {@link #createdEntry createdEntry}
     */
    public void freeEntry(BasicPoolEntry entry) {

        if (numEntries < 1) {
            throw new IllegalStateException("No entry created for this pool. "
                    + route);
        }
    ...
    /**
     * Indicates creation of an entry for this pool. The entry will <i>not</i>
     * be added to the list of free entries, it is only recognized as belonging
     * to this pool now. It can then be passed to {@link #freeEntry freeEntry}.
     * 
     * @param entry
    ...
    /**
     * Deletes an entry from this pool. Only entries that are currently free in
     * this pool can be deleted. Allocated entries can not be deleted.
     * 
     * @param entry
    ...
    /**
     * Forgets about an entry from this pool. This method is used to indicate
     * that an entry {@link #allocEntry allocated} from this pool has been lost
    ...
    /**
     * Adds a waiting thread. This pool makes no attempt to match waiting
     * threads with pool entries. It is the caller's responsibility to check
     * that there is no entry before adding a waiting thread.
     * 
     * @param wt
     *            the waiting thread
     */
