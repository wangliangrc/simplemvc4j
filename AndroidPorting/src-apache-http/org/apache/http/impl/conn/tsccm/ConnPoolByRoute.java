...
/**
 * A connection pool that maintains connections by route. This class is derived
 * from <code>MultiThreadedHttpConnectionManager</code> in HttpClient 3.x, see
 * there for original authors. It implements the same algorithm for connection
 * re-use and connection-per-host enforcement:
 * <ul>
 ...
 * {@link AbstractConnPool#poolLock poolLock} in the base class, not via
...
public class ConnPoolByRoute extends AbstractConnPool {
    ...
    /**
     * A map of route-specific pools. Keys are of class {@link HttpRoute},
     ...
     */
    public ConnPoolByRoute(final ClientConnectionOperator operator,
    ...
    /**
     * Creates the queue for {@link #freeConnections}. Called once by the
     * constructor.
     * 
     * @return a queue
     */
    ...
    /**
     * Creates the queue for {@link #waitingThreads}. Called once by the
     * constructor.
     * 
     * @return a queue
     */
    ...
    /**
     * Creates a new route-specific pool. Called by {@link #getRoutePool} when
     * necessary.
     * 
     * @param route
     *            the route
    ...
    /**
     * Creates a new waiting thread. Called by {@link #getRoutePool} when
     * necessary.
     * 
     * @param cond
     *            the condition to wait for
     * @param rospl
     *            the route specific pool, or <code>null</code>
     * 
     ...
     * 
     * @return the pool for the argument route, never <code>null</code> if
     *         <code>create</code> is <code>true</code>
     */
    ...
    @Override
    public PoolEntryRequest requestPoolEntry(final HttpRoute route,
            final Object state) {
     ...
     * {@link WaitingThread} is used to block,
     * {@link WaitingThreadAborter#setWaitingThread(WaitingThread)} must be
     * called before blocking, to allow the thread to be interrupted.
     * 
     * @param route
     ...
     * @param tunit
     *            the unit for the <code>timeout</code>, may be
     *            <code>null</code> only if there is no timeout
     * @param aborter
     *            an object which can abort a {@link WaitingThread}.
     * 
        ...
        try {
            ...
            while (entry == null) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Total connections kept alive: "
                            + freeConnections.size());
                    log.debug("Total issued connections: "
                            + issuedConnections.size());
                    log.debug("Total allocated connection: " + numConnections
                            + " out of " + maxTotalConnections);
                }
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Available capacity: " + rospl.getCapacity()
                            + " out of " + rospl.getMaxEntries() + " [" + route
                            + "][" + state + "]");
                }
    ...
    public void freeEntry(BasicPoolEntry entry, boolean reusable,
        ...
        try {
            ...
            if (reusable) {
                ...
                idleConnHandler.add(entry.getConnection(), validDuration,
                        timeUnit);
            } else {
                rospl.dropEntry();
                numConnections--;
     ...
     * 
     * @return an available pool entry for the given route, or <code>null</code>
        ...
        try {
            ...
            while (!done) {
                ...
                if (entry != null) {
                    ...
                    boolean valid = idleConnHandler.remove(entry
                            .getConnection());
                    if (!valid) {
                        ...
                        if (log.isDebugEnabled())
                            log.debug("Closing expired free connection" + " ["
                                    + rospl.getRoute() + "][" + state + "]");
                        closeConnection(entry.getConnection());
                        // We use dropEntry instead of deleteEntry because the
                        // entry
                        // is no longer "free" (we just allocated it), and
                        // deleteEntry
    ...
    /**
     * Creates a new pool entry. This method assumes that the new connection
     * will be handed out immediately.
     * 
     * @param rospl
     *            the route-specific pool for which to create the entry
     * @param op
    ...
    /**
     * Deletes a given pool entry. This closes the pooled connection and removes
     * all references, so that it can be GCed.
     * 
     * <p>
     * <b>Note:</b> Does not remove the entry from the freeConnections list. It
     * is assumed that the caller has already handled this step.
     * </p>
    ...
    /**
     * Delete an old, free pool entry to make room for a new one. Used to
     * replace pool entries with ones for a different route.
     */
    ...
    /**
     * Notifies a waiting thread that a connection is available. This will wake
     * a thread waiting in the specific route pool, if there is one. Otherwise,
     * a thread in the connection pool will be notified.
     * 
     * @param rospl
