...
/**
 * A connection "manager" for a single connection. This manager is good only for
 * single-threaded use. Allocation <i>always</i> returns the connection
 * immediately, even if it has not been released after the previous allocation.
 * In that case, a {@link #MISUSE_MESSAGE warning} is logged and the previously
 * issued connection is revoked.
 * <p>
 * This class is derived from <code>SimpleHttpConnectionManager</code> in
 * HttpClient 3. See there for original authors.
 * </p>
 * 
...
public class SingleClientConnManager implements ClientConnectionManager {
     ...
     * @param schreg
     *            the scheme registry, or <code>null</code> for the default
     *            registry
     */
     ...
     * 
     * @return a connection that can be used to communicate along the given
     *         route
     */
    ...
    public void releaseConnection(ManagedClientConnection conn,
        ...
        try {
            ...
            if (sca.isOpen()
                    && (this.alwaysShutDown || !sca.isMarkedReusable())) {
                if (log.isDebugEnabled()) {
            ...
            if (validDuration > 0)
                connectionExpiresTime = timeUnit.toMillis(validDuration)
                        + lastReleaseTime;
            else
    ...
    /**
     * Revokes the currently issued connection. The adapter gets disconnected,
