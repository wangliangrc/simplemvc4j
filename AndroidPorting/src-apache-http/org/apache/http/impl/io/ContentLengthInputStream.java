...
/**
 * Stream that cuts off after a specified number of bytes. Note that this class
 * NEVER closes the underlying stream, even when close gets called. Instead, it
 * will read until the "end" of its chunking on close, which allows for the
 * seamless execution of subsequent HTTP 1.1 requests, while not requiring the
 * client to remember to read the entire contents of the response.
 * 
 * <p>
