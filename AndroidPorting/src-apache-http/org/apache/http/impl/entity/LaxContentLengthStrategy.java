 ...
 * <p>
 * This strategy conforms to the entity transfer rules outlined in <a
 * href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec3.html#sec4.4">Section
 * 4.4</a>, <a
 ...
 * >Section 14.41</a> and <a
 * href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec3.html#sec14.13">Section
 * 14.13</a> of <a href="http://www.w3.org/Protocols/rfc2616/rfc2616.txt">RFC
 * 2616</a>, but is lenient about unsupported transfer codecs and malformed
 ...
 * applied. When a message-body is included with a message, the transfer-length
 ...
 * value other than "identity", then the transfer-length is defined by use of
 * the "chunked" transfer- coding (section 3.6), unless the message is
 ...
 * ransfer-length is not otherwise specified, then this self- elimiting media
 * type defines the transfer-length. This media type UST NOT be used unless the
 * sender knows that the recipient can arse it; the presence in a request of a
 * Range header with ultiple byte- range specifiers from a 1.1 client implies
 ...
 * used to indicate the end of a request body, since that would leave no
 ...
 * message-body MUST include a valid Content-Length header field unless the
 * server is known to be HTTP/1.1 compliant. If a request contains a
 * message-body and a Content-Length is not given, the server SHOULD respond
 * with 400 (bad request) if it cannot determine the length of the message, or
 * with 411 (length required) if it wishes to insist on receiving a valid
 * Content-Length.
 * </p>
 * <p>
 ...
 * HEAD method, the size of the entity-body that would have been sent had the
 * request been a GET.
 * </p>
 * 
 * <pre>
...
public class LaxContentLengthStrategy implements ContentLengthStrategy {
    ...
    public long determineLength(final HttpMessage message) throws HttpException {
        ...
        boolean strict = params
                .isParameterTrue(CoreProtocolPNames.STRICT_TRANSFER_ENCODING);

        Header transferEncodingHeader = message
        ...
        if (transferEncodingHeader != null) {
            ...
            if (HTTP.IDENTITY_CODING.equalsIgnoreCase(transferEncodingHeader
                    .getValue())) {
                return IDENTITY;
            } else if ((len > 0)
                    && (HTTP.CHUNK_CODING.equalsIgnoreCase(encodings[len - 1]
                            .getName()))) {
                return CHUNKED;
            } else {
                if (strict) {
            ...
            for (int i = headers.length - 1; i >= 0; i--) {
                ...
                // See if we can have better luck with another header, if
                // present
            }
            if (contentlen >= 0) {
                return contentlen;
