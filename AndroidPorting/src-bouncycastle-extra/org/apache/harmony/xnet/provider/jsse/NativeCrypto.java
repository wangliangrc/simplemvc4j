...
public final class NativeCrypto {
    ...
    public static native int EVP_PKEY_new_RSA(byte[] n, byte[] e, byte[] d,
            byte[] p, byte[] q);
    ...
    public static native void EVP_DigestUpdate(int ctx, byte[] buffer,
    ...
    private static int X509_NAME_hash(X500Principal principal, String algorithm) {
        try {
            byte[] digest = MessageDigest.getInstance(algorithm).digest(
    ...
    public static final Map<String, String> OPENSSL_TO_STANDARD_CIPHER_SUITES = new HashMap<String, String>();
    public static final Map<String, String> STANDARD_TO_OPENSSL_CIPHER_SUITES = new LinkedHashMap<String, String>();
    ...
    /**
     * TLS_EMPTY_RENEGOTIATION_INFO_SCSV is RFC 5746's renegotiation indication
     * signaling cipher suite value. It is not a real cipher suite. It is just
     * an indication in the default and supported cipher suite lists indicates
     * that the implementation supports secure renegotiation.
     * 
     * In the RI, its presence means that the SCSV is sent in the cipher suite
     * list to indicate secure renegotiation support and its absense means to
     * send an empty TLS renegotiation info extension instead.
     * 
     ...
     * renegotiation info if TLS is used (as opposed to SSL). So we simply allow
     * TLS_EMPTY_RENEGOTIATION_INFO_SCSV to be passed for compatibility as to
     * provide the hint that we support secure renegotiation.
     */
    ...
    static {
        ...
        // EXPORT1024 suites were never standardized but were widely
        // implemented.
        // OpenSSL 0.9.8c and later have disabled
        ...
        // PSK is Private Shared Key - didn't exist in Froyo's openssl - no JSSE
        // equivalent
        ...
        // Signaling Cipher Suite Value for secure renegotiation handled as
        // special case.
    ...
    static {
        ...
        STANDARD_TO_OPENSSL_CIPHER_SUITES.keySet().toArray(
    ...
    public static native void SSL_use_certificate(int ssl,
            byte[][] asn1DerEncodedCertificateChain);

    public static native void SSL_use_PrivateKey(int ssl,
            byte[] pkcs8EncodedPrivateKey);

    public static native void SSL_check_private_key(int ssl)
            throws SSLException;

    public static byte[][] encodeIssuerX509Principals(
            X509Certificate[] certificates) throws CertificateEncodingException {
        byte[][] principalBytes = new byte[certificates.length][];
        for (int i = 0; i < certificates.length; i++) {
            principalBytes[i] = certificates[i].getIssuerX500Principal()
                    .getEncoded();
        }
        return principalBytes;
    }

    public static native void SSL_set_client_CA_list(int ssl,
    ...
    public static void setEnabledProtocols(int ssl, String[] protocols) {
        ...
        // so first, assume we need to set all (disable all) and clear none
        // (enable none).
        // in the loop, selectively move bits from set to clear (from disable to
        // enable)
    ...
    public static String[] checkEnabledProtocols(String[] protocols) {
        ...
        for (int i = 0; i < protocols.length; i++) {
            String protocol = protocols[i];
            if (protocol == null) {
                throw new IllegalArgumentException("protocols[" + i
                        + "] == null");
            }
            if ((!protocol.equals(SUPPORTED_PROTOCOL_SSLV3))
    ...
    public static String[] checkEnabledCipherSuites(String[] cipherSuites) {
        ...
        for (int i = 0; i < cipherSuites.length; i++) {
            String cipherSuite = cipherSuites[i];
            if (cipherSuite == null) {
                throw new IllegalArgumentException("cipherSuites[" + i
                        + "] == null");
            }
            if (cipherSuite.equals(TLS_EMPTY_RENEGOTIATION_INFO_SCSV)) {
                continue;
    ...
    private static final String[] SUPPORTED_COMPRESSION_METHODS = {
            SUPPORTED_COMPRESSION_METHOD_ZLIB,
            SUPPORTED_COMPRESSION_METHOD_NULL };
    ...
    public static String[] checkEnabledCompressionMethods(String[] methods) {
        ...
        if (methods.length < 1
                && !methods[methods.length - 1]
                        .equals(SUPPORTED_COMPRESSION_METHOD_NULL)) {
    ...
    public static native void SSL_set_session(int sslNativePointer,
            int sslSessionNativePointer) throws SSLException;
    ...
    public static native void SSL_set_tlsext_host_name(int sslNativePointer,
    ...
    public static native int SSL_do_handshake(int sslNativePointer,
            FileDescriptor fd, SSLHandshakeCallbacks shc, int timeout,
            boolean client_mode) throws SSLException, SocketTimeoutException,
            CertificateException;

    /**
     * Currently only intended for forcing renegotiation for testing. Not used
     * within OpenSSLSocketImpl.
     */
    public static native void SSL_renegotiate(int sslNativePointer)
            throws SSLException;
     ...
     */
    public static native int SSL_read(int sslNativePointer, FileDescriptor fd,
            SSLHandshakeCallbacks shc, byte[] b, int off, int len, int timeout)
            throws IOException;
    ...
    public static native void SSL_write(int sslNativePointer,
            FileDescriptor fd, SSLHandshakeCallbacks shc, byte[] b, int off,
    ...
    public static native void SSL_shutdown(int sslNativePointer,
            FileDescriptor fd, SSLHandshakeCallbacks shc) throws IOException;

    public static native void SSL_free(int sslNativePointer);

    public static native byte[] SSL_SESSION_session_id(
    ...
    public static native String SSL_SESSION_compress_meth(
            int sslCtxNativePointer, int sslSessionNativePointer);
    ...
    /**
     * A collection of callbacks from the native OpenSSL code that are related
    ...
    public interface SSLHandshakeCallbacks {
         ...
         * 
         * @param asn1DerEncodedCertificateChain
         *            A chain of ASN.1 DER encoded certificates
         * @param authMethod
         *            auth algorithm name
         * 
        ...
        public void verifyCertificateChain(
                byte[][] asn1DerEncodedCertificateChain, String authMethod)
                throws CertificateException;

        /**
         * Called on an SSL client when the server requests (or requires a
         * certificate). The client can respond by using SSL_use_certificate and
         * SSL_use_PrivateKey to set a certificate if has an appropriate one
         * available, similar to how the server provides its certificate.
         * 
         * @param keyTypes
         *            key types supported by the server, convertible to strings
         *            with #keyType
         * @param asn1DerEncodedX500Principals
         *            CAs known to the server
         */
        ...
        /**
         * Called when SSL handshake is completed. Note that this can be after
         * SSL_do_handshake returns when handshake cutthrough is enabled.
         */
